...
class Matrix
{
     ...
     *
     * @return bool True if is passes verification after being converted
     */
    public function _assign($mText)
    {
        ...
        if (!preg_match("/^\[(([\-]*[0-9\. ]+[,]{0,1})+[;]{0,1})*\]$/", $mText)) {
            throw new \Exception("'{$mText}' is not a valid input", self::E_INVALID_INPUT);
        }
    ...
    public function _getN($mArray = false)
    {
        ...
        if ($this->isSquare($mArray)) {
            ...
            throw new \Exception("'{$m}' is not a square matrix", self::E_NOT_SQUARE);
        }
    }
    ...
    public function createIdentity($n, $useInternal = true)
    {
        ...
        if ($useInternal == true) {
            ...
            $nMatrix = new self($this->toString($mArray));
    ...
    public function toString($mArray = false)
    {
        ...
        if (is_array($mArray)) {
            ...
            throw new \Exception('No matrix to convert', self::E_NO_MATRIX);
        }
    }
    ...
    public function addMatrix(Matrix $nMatrix)
    {
        if (!$this->_verify() || !$nMatrix->_verify()) {
            throw new \Exception('Matrices have varying column sizes', self::E_INVALID_MATRIX);
        }
        ...
        if ((count($matrix1) != count($matrix2)) || (count($matrix1[0]) != count($matrix2[0]))) {
            ...
            throw new \Exception("The rows and/or columns '{$m1}' and '{$m2}' are not the same", self::E_NOT_EQUAL);
        }
        ...
        $rMatrix = new self($this->toString($rArray));
    ...
    public function mpScalar($k)
    {
        ...
        if (!$this->_verify()) {
            throw new \Exception("Matrix '{$this}' has varying column sizes", self::E_INVALID_MATRIX);
        }
    ...
    public function getDeterminant($mArray = false)
    {
        ...
        //print_r($mArray);
        if (!$this->isSquare($mArray)) {
            throw new \Exception("'{$this}' is not a square matrix", self::E_NOT_SQUARE);
        }
        ...
        if ($n < 1) {
            ...
            throw new \Exception('No Matrix', self::E_NO_MATRIX);
    ...
    public function transpose($cArray = false, $asArray = true)
    {
        ...
        if ($asArray == true) {
            ...
            $nMatrix = new self($this->toString($nArray));
    ...
    public function inverse($cArray = false)
    {
        ...
        if ($det == 0) {
            throw new \Exception("Determinant of {$this} is 0, No Inverse found", self::E_NO_INVERSE);
        }
    ...
    public function mpMatrix(Matrix $bMatrix)
    {
        if (!$this->_verify() || !$bMatrix->_verify()) {
            ...
            throw new \Exception("Either '{$eM1}' and/or '{$eM2}' is not a valid Matrix", self::E_INVALID_MATRIX);
            // @codeCoverageIgnoreEnd
        }
        $aArray = $this->matrix;
        ...
        if (count($aArray[0]) != count($bArray)) {
            $mA = $this->toString();
            $mB = $bMatrix->toString();
            throw new \Exception("Columns in '{$mA}' don't match Rows of '{$mB}'", self::E_NOT_EQUAL);
        }
